...
int main() {
	...
	printf("\n\n");
	//Reduce_temp동적할당
	Reduce_temp = malloc(sizeof(char*)*(num_terminal + 10));
	for (int i = 0; i < num_terminal; i++) {
		Reduce_temp[i] = malloc(sizeof(char)*TERMINAL_MAX_LENGTH);
	}
	...
	for (int i = 0; i < 10; i++) {
		...
		printf("SLR_table_ACTION[%d][%d]: %d  ", stack[top], SLR_ACTION_col, SLR_table_ACTION[stack[top]][SLR_ACTION_col]);
		...
		if (value_SLR_ACTION <= 0) {
			...
			//partition앞의 배열요소를 Reduce
			len_alpha = G_L_size[p_value_SLR_ACTION];
			partition_Reduce = partition - len_alpha;
			for (int row = 0; row < partition_Reduce; row++) {
				for (int col = 0; col <= input_length[row]; col++) {
					//printf("%c", input_string[row][col]);
					Reduce_temp[row][col] = input_string[row][col];
				}
			}
			...
			//A->alpha, A 를 Reduce_temp에 저장
			//printf("%s |", G_R[p_value_SLR_ACTION]);
			for (int i = 0;; i++) {
				Reduce_temp[partition_Reduce][i] = GOTO_Reduce[SLR_GOTO_col][i];
				if (Reduce_temp[partition_Reduce][i] == ',') {
					break;
				}
			}
			//partition뒷부분 저장
			for (int row = partition_Reduce + 1; row < num_terminal; row++) {
				for (int col = 0; col<=input_length[row]; col++) {
			...
			for (int row = 0; row < num_terminal; row++) {
				...
				for (int col = 0; Reduce_temp[row][col] != ','; col++) {
					printf("%c", Reduce_temp[row][col]);
				}
				printf(" ");
				if (row == partition_Reduce) {
					printf("|");
				}
			}
			printf("\n");
			...
			//partition앞까지 출력
			for (int row = 0; row <= partition - 1; row++) {
				for (int col = 0; input_string[row][col] != ','; col++) {
					printf("%c", input_string[row][col]);
				}
				printf(" ");
			}
